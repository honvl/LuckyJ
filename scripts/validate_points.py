...
REVIEW_EXAMPLES = {
    "point-01": (
        "Review example: in a South-round placement hand, a low-risk tile that keeps a clean fold or cheap win can beat a thin value upgrade, but only when it still answers real danger.",
        "検討例: 南場の着順手では、薄い打点上昇より、きれいな撤退や安い和了を残す低危険牌が勝つことがある。ただし実際の危険に効く時だけ。",
    ),
    "point-02": (
    ...
    "point-03": (
        "Review example: call a bad block only when it creates yaku, tenpai, denial, tempo pressure, or safe draw equity; skip the call that merely exposes a fragile one-away hand.",
        "検討例: 悪いブロックは、役、テンパイ、阻止、テンポ圧、安全なツモ番を作る時だけ鳴く。壊れやすい一向聴を晒すだけなら見送る。",
    ),
    "point-04": (
        "Review example: after opening, keep a safe tile for the live threat if the hand can still reach tenpai; commitment still needs one clean brake.",
        "検討例: 副露後でもテンパイが現実的なら、実脅威への安全牌を一枚残す。コミットした手にもきれいなブレーキを残す。",
    ),
    "point-05": (
    ...
    "point-14": (
        "Review example: keep genbutsu or suji only after naming the target player and the next turn it protects; untargeted safety is just clutter.",
        "検討例: 現物や筋は、対象者と守る次巡を名付けてから残す。対象のない安全牌は余り牌として見る。",
    ),
    "point-15": (
    ...
    "point-18": (
        "Review example: classify each honor as self value, opponent yaku condition, dead material, or defensive tile before cutting.",
        "検討例: 字牌は切る前に、自分の打点、相手の役条件、枯れ材料、守備牌へ分類する。",
    ),
}
...
def build() -> dict[str, Any]:
    ...
    points = [
        point(
            ...
            "Qualified",
            "条件付き",
            "Placement framing is valid, but it is only useful when it changes concrete danger, value, or next-turn safety.",
            "着順から読む方針は妥当。ただし具体的な危険度、打点、次巡安全が変わる時だけ有効。",
            "Leader safety is a qualifier, not a standalone excuse to kill shape.",
            "トップ目安全は単独ルールではなく、形を壊す理由にはならない条件として扱う。",
            "Example: as leader, a low-danger discard that preserves the next safe turn can beat a thin value upgrade when it still fits the hand's placement job.",
            "例: トップ目では、着順上の仕事に合い次巡安全も残す低危険打牌が、薄い打点上昇より勝つことがある。",
            [
        ...
        point(
            ...
            "Qualified",
            "条件付き",
            "Aggregate results support purposeful calling; bad-shape repair and tempo belong under the same named-purpose rule.",
            "目的のある鳴きは成績面で支持される。悪形修復とテンポ鳴きは同じ目的付きルールで扱う。",
            "Demand a named purpose: tenpai, yaku creation, value, denial, or safe-tenpai equity.",
            "テンパイ、役作り、打点、阻止、形式テンパイなど目的を名付ける。",
            "Example: call when it creates a real yaku route, changes the round clock, or gives a safe tenpai path from the weak block.",
            "例: 弱ブロックから役ルート、局の時計変更、安全テンパイルートが生まれる時だけ鳴く。",
        ...
        point(
            ...
            "Calls and Defense",
            "鳴きと守備",
            "Open hands still need safe tiles.",
            "副露手にも安全牌を残す。",
            "strong",
            "Strong",
            "強い",
            "Threat-specific safe-tile retention was strong, while generic safe-tile hoarding was worse than baseline.",
            "脅威に対応する安全牌保持は強い一方、一般的な安全牌抱えは基準より悪い。",
            "This point is target-specific: name the threat before keeping the tile.",
            "対象特化のルール。牌を残す前に相手を名付ける。",
        ...
        point(
            ...
            [
                ...
                pattern(patterns, "multi_threat_safe_tenpai", "Multi-threat tiles", "多件脅威安全牌"),
            ],
        ),
        point(
            "point-10",
            ...
            [late_precision, pattern(patterns, "multi_threat_safe_tenpai", "Late/middle multiple-threat tiles", "中終盤の多件脅威安全牌"), mismatch],
        ),
        point(
            "point-11",
            "Late Game",
            ...
            "Chase keiten through multiple live threats only with safe tiles.",
            "複数脅威への形式テンパイは安全牌がある時だけ追う。",
            ...
            [drawn_hand, pattern(patterns, "multi_threat_safe_tenpai", "Safe-tenpai style tiles", "安全テンパイ型の安全牌"), late_precision],
        ),
        point(
            "point-12",
            "Review",
        ...
        point(
            ...
            "Defense",
            "守備",
            "Keep genbutsu and suji tiles for a named target.",
            "現物と筋の牌は対象を名付けて残す。",
            "strong",
            "Strong",
            "強い",
            "Threat-specific safety is strongly supported; generic safety retention only works after naming the target.",
            "対象付き安全牌保持は強く支持される。一般的な安全牌保持は対象を名付けて初めて使える。",
            "The point must stay narrow: who is the tile for, when will it be spent, and what shape cost does it impose?",
            "対象、使う時期、手牌が払う損を必ず決める狭いルールにする。",
            "Example: keep suji only if it defends the current riichi or open hand, and spend it when its live-threat target expires.",
            "例: 現在のリーチ者や副露者に通る筋なら残し、対象が切れたら使う。",
            [
                pattern(patterns, "threat_keep_exit", "Tile for active threat", "実脅威への安全牌"),
        ...
        point(
            ...
            "Self yakuhai, opponent yaku condition, dead honor, and safe tile are different categories.",
            "自分役牌、相手の役条件、枯れ字牌、安全牌は別カテゴリ。",
            "Example: a lone dragon can be value seed, opponent yaku-condition liability, or safe tile depending on calls and rivers.",
            "例: 孤立三元牌は、鳴きと河次第で打点種、相手役条件、安全牌のどれにもなる。",
            [
                pattern(patterns, "honor_cleanup_vs_shape", "Loose honor cleanup", "浮き字牌整理"),
