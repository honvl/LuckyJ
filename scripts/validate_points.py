#!/usr/bin/env python3
"""Validate the numbered LuckyJ playbook points against mined statistics."""
...
def build() -> dict[str, Any]:
    ...
    call_riichi = aggregate(
        f"Winning-half volume was not passive: calls/game {num(top['calls_per_game'])} vs {num(bottom['calls_per_game'])}, riichi/game {num(top['riichi_per_game'])} vs {num(bottom['riichi_per_game'])}.",
        f"上位半分は受け身ではない: 鳴きは半荘あたり {num(top['calls_per_game'])} 対 {num(bottom['calls_per_game'])}、リーチは {num(top['riichi_per_game'])} 対 {num(bottom['riichi_per_game'])}。",
    )
    call_rate = aggregate(
        f"Call opportunities became calls {pct(summary['call_round_rate'])} of hands, so call points need purpose filters instead of a yes/no rule.",
    ...
    middle_late = aggregate(
        f"Middle plus late decisions total {middle['decisions'] + late['decisions']:,}; the defense points are not based only on opening-row cleanup.",
        f"中盤と終盤の判断は合計 {middle['decisions'] + late['decisions']:,} 件。守備点は序盤整理だけではない。",
    )
    ...
    points = [
        point(
            ...
            "Value and Placement",
            "打点と着順",
            "Start with the placement problem, not the hand problem.",
            "手牌問題ではなく、着順問題から始める。",
            "qualified",
            "Qualified",
            "条件付き",
            ...
            "Require concrete upside and a discard that does not add unmanaged danger.",
        ...
        point(
            ...
            "The support is for anchors with a job, not for every pair or every dora tile.",
            "仕事のある支点への支持であり、全ての対子やドラを残す話ではない。",
        ...
        point(
            ...
            "鳴きと役条件",
            "Do not call bad shapes unless the call changes the hand's job.",
            "悪形は、手の仕事が変わる時だけ鳴く。",
            "qualified",
            "Qualified",
            "条件付き",
            "Aggregate results support purposeful calling, but the data does not validate a broad call-more rule.",
            "目的のある鳴きは成績面で支持されるが、単純な鳴き増やしは検証されていない。",
            "Demand a named purpose: tenpai, yaku creation, value, denial, or safe-tenpai equity.",
            "テンパイ、役作り、打点、阻止、形式テンパイなど目的を名付ける。",
            "Example: chi only when it creates a real yaku route or a safe tenpai path, not just because the block is weak.",
            "例: 弱ブロックだから鳴くのではなく、役ルートや安全テンパイルートが生まれる時だけチーする。",
        ...
        point(
            ...
            "Strong",
            "強い",
            "Safer-than-NAGA choices were one of the cleanest mined signals; danger pricing should be explicit.",
            "NAGA より安全な選択は最もきれいな信号の一つで、危険度の値付けを明示すべき。",
            "Do not turn this into timid early slimming when the tile still carries value or yaku routes.",
            "価値や役ルートが残る牌まで臆病に早切りしない。",
        ...
        point(
            ...
            "Qualified",
            "条件付き",
            "Dora/red retention is supported; broad risk-buying for value is not.",
            "ドラ/赤の保持は支持されるが、打点のための広いリスク購入は支持されない。",
            "The value seed must improve a real winning route, not just make the hand prettier.",
        ...
        point(
            ...
            "Qualified",
            "条件付き",
            "The outcome split supports active conversion, but the call data is not a causal proof.",
            "成績差は能動的な変換を支持するが、鳴きデータだけでは因果証明ではない。",
            "Use this as a review question, not as permission to call every speed tile.",
            "スピード牌を全部鳴く許可ではなく、復習時の問いとして使う。",
        ...
        point(
            ...
            "Value and Pressure",
            "打点と圧力",
            "Riichi is a conversion tool, not a default.",
            "リーチは変換手段であり、自動ではない。",
            "qualified",
            "Qualified",
            "条件付き",
            "Top-half games used more riichi, but the point is conversion quality rather than raw reach volume.",
            "上位半分はリーチ回数も多いが、要点は回数ではなく変換品質。",
        ...
        point(
            ...
            "Qualified",
            "条件付き",
            "Late positions are numerous enough to validate as a training category, but not every late LuckyJ split is good.",
            "終盤局面は訓練カテゴリにできる母数があるが、全ての終盤 LuckyJ 分岐が良いわけではない。",
        ...
        point(
            ...
            "Do not chase keiten through multiple live threats without safe exits.",
            "複数脅威に対して出口なしで形式テンパイを追わない。",
        ...
        point(
            ...
            "Review",
            "復習",
            "Use model disagreement as a prompt, not a verdict.",
            "モデル不一致は結論ではなく問い。",
            "strong",
            "Strong",
            "強い",
            "The mismatch base is large, and most mismatches are not severe by the NAGA proxy.",
            "不一致母数は大きく、NAGA proxy 上で重い不一致は少数にとどまる。",
            "A split marks a hand to study; it does not prove either side at sight.",
            "分岐は復習対象を示すだけで、その場でどちらかの正解を証明しない。",
        ...
        point(
            ...
            "Calls and Yaku Conditions",
            "鳴きと役条件",
            "Do not feed uncertain open-hand yaku conditions.",
            "未確定副露に役条件を渡さない。",
            "strong",
            "Strong",
            "強い",
            "Loose honor cleanup is one of the strongest mined signals.",
            "浮き字牌整理は最も強い mined signal の一つ。",
        ...
        point(
            ...
            "Strong",
            "強い",
            "Threat-specific safety is strongly supported; generic safety retention is not.",
            "対象付き安全牌保持は強く支持されるが、一般的な安全牌保持は支持されない。",
            ...
            [
                pattern(patterns, "threat_keep_exit", "Exit versus active threat", "実脅威への出口"),
        ...
        point(
            ...
            "The reverse middle-cut pattern was much worse, so do not confuse route preservation with vague safety.",
            "逆の中張牌切りは大きく悪いので、曖昧な守備と混同しない。",
        ...
        point(
            ...
            "元の安全牌表現への最も明確な修正。",
            "The same tile flips from good to bad when it is not tied to a live threat.",
            "実脅威に結び付かない時、同じ安全牌でも良し悪しが反転する。",
            "Example: label a kept 9m as genbutsu to the dealer riichi, not merely 'safe'.",
            "例: 残す9萬を単に安全牌ではなく、親リーチへの現物として名付ける。",
            [
        ...
        point(
            ...
            "Strong",
            "強い",
            "Honor cleanup and pair-anchor stats support role labeling rather than a single honor rule.",
            "字牌整理と対子支点の統計は、単一の字牌ルールではなく役割名付けを支持する。",
        ...
        point(
            ...
            "着順と守備",
            "Leader safety is a qualifier, not a plan.",
            "トップ目安全は条件であって計画ではない。",
            "qualified",
            "Qualified",
            "条件付き",
            "Leader low-risk choices were only mildly better than baseline; the stronger signal is avoiding extra danger.",
            "トップ目の低危険選択は平均との差が小さい。より強い信号は余計な危険を増やさないこと。",
            "Do not lower ambition unless the discard also keeps the next turn playable.",
            "次巡も打てる形を保つ時だけ目標を下げる。",
            "Example: choose the low-risk tile that keeps a clean fold or cheap win, not a tile that simply avoids building value.",
            "例: 打点を作らないだけの牌ではなく、安い和了かきれいな撤退を残す低危険牌を選ぶ。",
            [
